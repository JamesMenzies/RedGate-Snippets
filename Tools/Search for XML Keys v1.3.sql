--
--<auditElement><field name="CodeTypeID"><oldValue>1000204</oldValue>
--</field><field name="Name"><oldValue>Histology</oldValue>
--</field><field name="Order"><oldValue>70</oldValue>
--</field><field name="IsActive"><oldValue>True</oldValue></field>
--<field name="UpdateInSearchEngine"><oldValue>False</oldValue>
--</field><field name="OIHiliteStyleID"><oldValue>3</oldValue></field><field name="KeyboardShortcut"><oldValue /></field><field name="RelativityApplications"><oldValue /></field><field name="IsPreviewUser"><oldValue>False</oldValue></field><field name="ParentArtifactID"><oldValue>1003663</oldValue></field><field name="AccessControlListID"><oldValue>1</oldValue></field><field name="AccessControlListIsInherited"><oldValue>True</oldValue></field><field name="CreatedBy"><oldValue>1167819</oldValue></field><field name="LastModifiedBy"><oldValue>1167819</oldValue></field><field name="CreatedOn"><oldValue>2015-11-26T12:32:39</oldValue></field><field name="LastModifiedOn"><oldValue>2015-12-01T18:35:55</oldValue></field><field name="TextIdentifier"><oldValue>Histology</oldValue></field><field name="ContainerID"><oldValue>1003663</oldValue></field><field name="ArtifactID"><oldValue>1061609</oldValue></field><field name="Keywords"><oldValue /></field><field name="Notes"><oldValue /></field><field name="DeleteFlag"><oldValue>False</oldValue></field><field name="Guids"><oldValue>b50cce34-1d3e-413f-99a0-680ce7eed494</oldValue></field></auditElement>
...
DECLARE	@SearchFor VARCHAR(255)
SET @SearchFor = '1061609'
--CRM exmaple May Street	
--general example James
--**********************************************
	...
	BEGIN
		...
		BEGIN
			...
			SET @baby = 'SELECT [TableName]=''' + @TableName + ''', [ColumnName] = ''' + @ColumnName + ''', tbl.*  FROM ' + @TableName + ' tbl (NOLOCK) '
				+ ' WHERE ' + @ColumnName + '.exist(''//*[contains(., ''''' + CAST(@SearchFor AS NVARCHAR(255)) + ''''')]'') = 1'
			--build a query to TEST if the query is needed. AND report on hits.				   --.exist('//*[contains(., ''1061609'')]')  =1
			SET @localquery = 'IF EXISTS (SELECT 1 FROM ' + @TableName + ' WHERE ' + @ColumnName + '.exist(''//*[contains(., ''''' + CAST(@SearchFor AS NVARCHAR(255)) + ''''')]'') = 1)'
				+ ' SELECT  [TableName]=''' + @TableName + ''', [ColumnName] = ''' + @ColumnName + ''', query =  @baby, hits = ' + '(SELECT COUNT('
				+ @ColumnName + ') FROM ' + @TableName + ' WHERE ' + @ColumnName + '.exist(''//*[contains(., ''''' + CAST(@SearchFor AS NVARCHAR(255)) + ''''')]'') = 1)'
			PRINT @localquery
			--RUN local query 
