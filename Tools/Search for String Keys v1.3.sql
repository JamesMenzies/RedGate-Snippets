...
NOTES
	Loops over tables TEXT (char, varchar, nvarchar,etc) type Columns columns Searching for all values LIKE the search string
	...
	BEGIN
		...
		BEGIN
			...
			SET @baby = 'SELECT [TableName]=''' + @TableName + ''', [ColumnName] = ''' + @ColumnName + ''', tbl.*  FROM ' + @TableName + ' tbl (NOLOCK) '
				+ ' WHERE ' + @ColumnName + ' LIKE ''' + CAST(@SearchFor AS NVARCHAR(255)) +''''
			--build a query to TEST if the query is needed. AND report on hits.
			SET @localquery = 'IF EXISTS (SELECT 1 FROM ' + @TableName + 'WHERE ' + @ColumnName + ' LIKE ''' + CAST(@SearchFor AS NVARCHAR(255)) + ''')'
				+ ' SELECT  [TableName]=''' + @TableName + ''', [ColumnName] = ''' + @ColumnName + ''', query =  @baby, hits = ' + '(SELECT COUNT('
				+ @ColumnName + ') FROM ' + @TableName + 'WHERE ' + @ColumnName + ' LIKE ''' + CAST(@SearchFor AS NVARCHAR(255)) + ''')'
			--PRINT @localquery
			--RUN local query 
